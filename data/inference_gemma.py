...
def gcp_inference_gemma(text, pipe):
    ...
    try:
        outputs = pipe(messages, max_new_tokens=2048, do_sample = False)  
...
def process_g2p(data, model_id, pipe):
    ...
    for item in tqdm(data, desc=f"Inference with {model_id}"):
        ...
        model_output = gcp_inference_gemma(question, pipe)
...
if __name__ == "__main__":
    ...
    # 인퍼런스 실행
    df_results = process_g2p(processed_data, args.model_id, pipe)
